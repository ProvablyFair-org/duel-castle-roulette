/**
 * Statistical utilities for Castle Roulette audit.
 * Chi-squared, autocorrelation, Wald-Wolfowitz runs test.
 * Adapted from duel-crash reference implementation.
 */
...
export function runsTest(binary: boolean[]): { runs: number; expected: number; z: number; pValue: number } {
  const n = binary.length;
  if (n < 10) return { runs: 0, expected: 0, z: 0, pValue: 1 };
  ...
  const n2 = n - n1;
  if (n1 === 0 || n2 === 0) return { runs, expected: 1, z: 0, pValue: 1 };
  ...
  return { runs, expected: expectedRuns, z, pValue };
}
